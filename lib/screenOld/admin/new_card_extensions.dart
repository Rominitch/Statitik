...
class _NewCardExtensionsState extends State<NewCardExtensions> {
  ...
  bool _modify = false;
  PokemonCardExtension data = PokemonCardExtension.creation(PokemonCardData(0, [], Level.base, TypeCard.plante, CardMarkers.from([])), Environment.instance.collection.rarities[Collection.idEmptyRarity]!, Environment.instance.collection.sets);
  int idList = 0;
  ...
  void onAddCard(int listId, int? pos) {
    setState((){
      ...
      // Create new card
      var newItem = PokemonCardExtension.creation(PokemonCardData(0, [], data.data.level, data.data.type, CardMarkers.from([])),
