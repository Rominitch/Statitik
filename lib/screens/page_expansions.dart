...
class _PageExpansionsState extends State<PageExpansions> {
  ...
  Future<void> _waitStats(refresh) async {
    final sData = _statsData;
    ...
    // Get data from DB
    final expansion = sData.selection.expansion!;

    final user  = Environment.instance.user;
    final stats = Environment.instance.pkStatistics();
    stats.getStatsFrom(Environment.instance.pkDB(), expansion).then( (PokeStatsBooster boosterStats) {
      sData.stats = boosterStats;
      // Compute Cards stats
      /*
      int idCard=0;
      for (final listCardSE in expansion.cards.cards) {
        for (final cardSE in listCardSE) {
          sData.cardStats.stats!.add(expansion, cardSE, CardIdentifier.from([0, idCard, 0]));
          idCard +=1;
        }
      }
      */
      // Get user info after
      if(user != null) {
        stats.getStatsFrom(Environment.instance.pkDB(), expansion, /*product, sData.category,*/ user.idDB).then( (PokeStatsBooster ustats) {
            ...
            child: Padding(
              ...
              child: WidgetCardVersionSelector(_statsData.selection,
                onPress: () {
                  ...
                  if(_statsData.selection.expansion != null) {
                    _pageController.jumpToPage(_se.indexOf(_statsData.selection.expansion!));
                  }
                }
              )
            )
